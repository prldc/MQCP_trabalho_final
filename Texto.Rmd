...
## 1. Mídias digitais: campo político e ferramenta de pesquisa cada vez mais relevante
...
A Modelagem de Tópicos é utilizada para se ter uma ideia inicial do que os dados estão tratando, sem a necessidade de uma leitura prévia (STAUDT JUNIOR, 2016). Consiste em um método para classificação não supervisionada, que encontra grupos naturais de palavras, mesmo quando não temos certeza do que estamos procurando (SILGE; ROBINSON, 2017). É uma ferramenta útil para exploração de dados, nos quais os tópicos proveem um resumo dos dados textuais, algo que seria impossível de se obter manualmente (SCHNEIDER, 2014). 
...
Para facilitar a compreensão do método aplicado, dividimos o processo de análise dos dados em 5 fases:
...
<colar código de merge_tweets.R>

Formada a base de dados, podemos visualizar facilmente como os tweets se distribuíram ao longo do tempo. É possível observar na figura abaixo que o período eleitoral, em laranja, foi o período de maior atividade nos candidatos no Twitter:

**Imagem Aqui**

# 2.2.	Vetorização 

Após a coleta dos dados brutos, deixamos o conjunto de dados em um formato para que pudesse ser processado no R. Dessa forma, vetorizamos o corpus por meio de um processo chamado de tokenização, que consiste em converter as sentenças de texto em vetores numéricos. Para isso, utilizamos a técnica de saco de palavras (“bag of words”), uma representação simplificada dos dados textuais, que faz um processo de contagem, mas qualquer informação sobre a gramática ou a posição das palavras é descartada. O saco de palavras é transformado em um conjunto de termos e suas respectivas frequências^[https://minerandodados.com.br/como-preparar-dados-de-texto-para-machine-learning/ Acessado em 16/12/2020.] . Dessa forma, a frase “isso é um exemplo” seria um vetor linha com quatro colunas, com o valor 1 para cada palavra. Confira-se a seguinte figura:

**Outra Imagem Aqui**

# 2.3.	Tratamento dos dados 
Após a vetorização dos dados coletados do twitter, foi realizada uma limpeza deles, removendo os dados desnecessários do corpus, como palavras impedidas (stopwords) (SILGE; ROBINSON, 2017). Essas palavras são aquelas muito frequentes no texto, mas que geram poluição na base de dados. Essas palavras são removidas para manter as distintivas, que ajudarão na composição dos tópicos do nosso exercício.

# 2.4	Cálculo da matriz de frequência dos documentos 
...
# 2.5. Aplicação da modelagem STM (Structural Topic Model) 
